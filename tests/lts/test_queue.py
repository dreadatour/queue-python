...
class TestSuite_Basic(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.queue = Queue("127.0.0.1", 33015, 0)
...
class TestSuite_00_ConnectionTest(TestSuite_Basic):
    def test_00_ConProp(self):
        conn = self.queue.tnt
        self.assertTrue(isinstance(conn, tarantool.lts.connection.Connection))
        self.assertTrue(
            isinstance(self.queue.tnt, tarantool.lts.connection.Connection)
        )
    ...
    def test_03_TaskMeta(self):
        ...
        self.assertEqual(task_meta.keys(), [
            'status', 'task_id', 'cid', 'ttr', 'tube', 'created', 'pri',
            'ctaken', 'ipri', 'cbury', 'ttl', 'now', 'event'
        ])
        self.tube.take().ack()
...
class TestSuite_03_CustomLockAndConnection(TestSuite_Basic):
    def test_00_GoodLock(self):
        ...
        self.assertTrue(
            isinstance(self.queue.tarantool_lock, type(threading.Lock()))
        )
    ...
    def test_01_GoodConnection(self):
        ...
        self.queue.tarantool_connection = tarantool.lts.Connection
        ...
        self.assertFalse(hasattr(self.queue, '_tnt'))
        self.assertEqual(
            self.queue.tarantool_connection, tarantool.lts.Connection
        )
